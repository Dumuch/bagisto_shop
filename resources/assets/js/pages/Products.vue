...
export default {
    ...
    mounted() {
        apiToLocalStorage('allProducts', 'products').then(res => this.products = res)
        apiToLocalStorage('allCategories', 'categories').then(res => this.categories = res)
    },
    ...
    methods: {
        ...
        filteredProducts() {
            ...
            if (this.search) {
                arr = arr.filter((item) => item.name.toLowerCase().indexOf(this.search.toLowerCase()) >= 0)
            }
            return arr
        }
